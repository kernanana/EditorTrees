...
//		while (true) { /* spin until user closes graphic window */ }
	...
	public void test272AddManyRandom() {
		...
		// TODO: change to new Random(17), new Random(50), etc. while debugging 
		...
		for (int k = 0; k < NUM_NODES / SKIP_INTERVAL; k++) {
			for (int j = 0; j < 10; j++) {
				...
				// TODO: You can uncomment and modify the following code as needed if you find bugs 
				// that you need to track down.
				if (!t.balanceCodesAreCorrect()) {
					String s = String.format(
							"Balance codes incorrect on tree with % nodes",
							t.slowSize()
					);
				...
				if (!t.ranksMatchLeftSubtreeSize()) {
					String s = String.format(
							"Ranks incorrect on tree with % nodes",
							t.slowSize()
					);
					System.out.println(s);
					// Same here.
	...
	public void test280ConstructorWithEditTree() {
		...
		System.out.print(t2.slowHeight());
		System.out.print(maxHeight(t2.size()));
